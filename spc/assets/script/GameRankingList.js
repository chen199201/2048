if (CC_WECHATGAME) {
	if (window.wx.getSystemInfoSync().platform != 'ios') {
		...
		cc.view._convertTouchesWithScale = function (touches) {
			var viewport = this._viewPortRect,
				scaleX = this._scaleX / 2,
				scaleY = this._scaleY / 2,
				selTouch,
				selPoint,
				selPrePoint;
					...
					success: (res) => {
						...
						for (let i = 0; i < data.length; i++) {
							...
							if (data[i].avatarUrl == userData.avatarUrl) {
								...
								userItem.y = 5;
								userItem.children[4].getComponent(cc.Sprite).spriteFrame = this.hmask
								userItem.getComponent('RankItem').init(i, playerInfo);
								this.ownRank.addChild(userItem, 1, 1000);
								console.log(userItem);
								userItem.children[5].active = false;
							}
						}
						lastRank = onther.pop();
						for (let i = 0; i < lastArr.length; i++) {
							if (lastarr[i].children[0].getComponent(cc.Label).string == lastRank.children[0].getComponent(cc.Label).string) {
								lastarr[i].children[5].active = false;
							}
						}
					},
					fail: (res) => {
			...
			success: (userRes) => {
				...
				for (let i = 0; i < 40; i++) {
					...
					if (data[i].avatarUrl == userData.avatarUrl) {
						let userItem = cc.instantiate(this.prefabRankItem);
						userItem.y = 5;
						userItem.getComponent('RankItem').init2(data[i].num || i + 1, playerInfo);
						this.ownRank.addChild(userItem, 1, 1000);
						data.pop();
						userItem.children[5].active = false;
						console.log(userItem);
				...
				if (data[40]) {
					var playerInfo = data[40];
					let userItem = cc.instantiate(this.prefabRankItem);
					userItem.y = 5;
					userItem.getComponent('RankItem').init2(data[40].num, playerInfo);
					this.ownRank.addChild(userItem, 1, 1000);
					userItem.children[5].active = false;
				}
			},
